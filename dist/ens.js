...
import { getENSContract, getResolverContract, getReverseRegistrarContract } from './contracts';
import { emptyAddress, getEnsStartBlock, labelhash, namehash, uniq } from './utils';
import { decodeContenthash, encodeContenthash } from './utils/contents';
import { encodeLabelhash } from './utils/labelhash';
import { getAccount, getNetworkId, getProvider, getSigner, getWeb3 } from './web3';
...
export class ENS {
  constructor({
    ...
    const hasRegistry = this.contracts[networkId] && Object.keys(this.contracts[networkId]).includes('registry');
  ...
  async getSubdomains(name) {
    ...
    const ownerPromises = labelhashes.map(label => this.getOwnerWithLabelHash(label, namehash));
