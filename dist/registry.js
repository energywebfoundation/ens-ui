import { getENS, getNamehash, getLabelhash, getENSEvent, getReverseRegistrarContract, getResolverContract, getTestRegistrarContract, getNamehashWithLabelHash, normalize } from './ens';
import { decryptHashes } from './preimage';
import { uniq, ensStartBlock, checkLabels, mergeLabels, emptyAddress, isDecrypted } from './utils';
  ...
  try {
    ...
    if (isContentHashSupported) {
      const {
        ...
        decoded,
        error
      } = decodeContenthash(await Resolver.contenthash(namehash));
...
export async function setTextWithResolver(name, key, recordValue, resolverAddr) {
  const namehash = getNamehash(name);
  const ResolverWithoutSigner = await getResolverContract(resolverAddr);
  const signer = await getSigner();
  const Resolver = ResolverWithoutSigner.connect(signer);
...
export async function getDomainDetails(name) {
  ...
  const [owner, resolver] = await Promise.all([getOwner(name), getResolver(name)]);
  const node = {
    name,
    label: nameArray[0],
    labelhash,
...
export async function registerTestdomain(label) {
  ...
  } = await getTestRegistrarContract();
  const signer = await getSigner();
  const labelhash = getLabelhash(label);
  const account = await getAccount();
